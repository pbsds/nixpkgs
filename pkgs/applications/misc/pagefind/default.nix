...
rustPlatform.buildRustPackage rec {
  ...
  postPatch = ''
    ...
    (
      ...
      if [ -n "${builtins.toString stdenv.isDarwin}" ]; then
          httplz_port=$(python -c 'import socket; s=socket.socket(); s.bind(("", 0)); print(s.getsockname()[1]); s.close()')
      fi
      httplz --port $httplz_port -- .lindera-http-plz/ &
      httplz_pid=$!
      echo $httplz_pid >$TMPDIR/.httplz_pid
